...
CRITICAL FIX: Array synchronization validation and proper ID mapping to fix search issues
...
try:
    ...
    # Look for .env in parent directory since we're in knowledge-base/
...
# Create a separate logger for progress that bypasses the warning filter
...
def process_methodology_worker(args):
    """Worker function for parallel methodology processing"""
    worker_id, files_batch, temp_dir, use_openai = args
    
    try:
        # Initialize embedding model based on use_openai flag
        if use_openai:
            if not OPENAI_AVAILABLE:
                raise ImportError("OpenAI package required for --use-openai")
            api_key = os.getenv('OPENAI_API_KEY')
            if not api_key:
                raise ValueError("OPENAI_API_KEY environment variable required")
            openai_client = OpenAI(api_key=api_key)
            model = openai_client  # Pass OpenAI client as "model"
        else:
        ...
        for file_path in files_batch:
            try:
                chunks = process_single_file(file_path, model, use_openai)
                worker_results.extend(chunks)
                files_processed += 1
        ...
        temp_file = temp_dir / f"worker_{worker_id}.json"
        with open(temp_file, 'w') as f:
            json.dump(worker_results, f)
        
        return {'files_processed': files_processed, 'chunks_created': chunks_created, 'errors': errors}
        
    except Exception as e:
        logger.error(f"Worker {worker_id} failed: {e}")
        return {'files_processed': 0, 'chunks_created': 0, 'errors': len(files_batch)}

def process_single_file(file_path: Path, model, use_openai: bool = False) -> List[Dict]:
    ...
    try:
        ...
        if use_openai:
            # Bulletproof OpenAI embeddings with caching and token limits
            ...
            for i in range(0, len(chunk_texts), batch_size):
                ...
                for text in batch_texts:
                    ...
                    if cache_file.exists():
                        ...
                        try:
                            response_embeddings = create_embeddings_with_retry(model, [text])
                ...
                if current_batch % 5 == 0 or current_batch == 1 or current_batch == total_batches:
                    show_progress(current_batch, total_batches, "OpenAI Embedding")
...
def extract_headings_and_sections(text: str, file_type: str) -> List[Dict]:
...
class ConceptBasedKnowledgeBuilder:
    ...
    def _init_model(self):
        ...
        if not self.use_openai:
            ...
            logger.info(f"🔄 Will use OpenAI embeddings API")
    ...
    def _generate_sentence_transformer_embeddings(self, texts: List[str]) -> np.ndarray:
        ...
        return self.embedding_model.encode(texts, show_progress_bar=False, convert_to_numpy=True).astype(np.float32)
    ...
    def build_knowledge_bases(self, rebuild: bool = False):
        ...
        print(f"🚀 Starting concept-based dual-path knowledge base build...")
    ...
    def _build_variables_faiss(self, canonical_path: Path):
        ...
        logger.info(f"✅ Synchronized arrays: IDs, metadata, embeddings all length {len(all_variable_ids)}")
    ...
    def _process_methodology_parallel(self, doc_files: List[Path]):
        ...
        logger.info(f"🔄 Processing methodology files with {self.workers} workers...")
    ...
    def _merge_methodology_results(self, temp_dir: Path):
        """Merge worker results into ChromaDB"""
        logger.info("🔗 Merging worker results into methodology database...")
