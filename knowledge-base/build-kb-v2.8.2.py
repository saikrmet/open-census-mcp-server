#!/usr/bin/env python3
"""
OpenAI Knowledge Base Builder v2.8.2 - Cross-File Batching Optimized
HIGH PERFORMANCE FEATURES:
- Cross-file batching: workers collect chunks from multiple files, then batch embed
- File filtering: skip files <1KB with detailed skip log
- Reduced logging: progress every 50 files, not every file
- Massive embedding batches for maximum API efficiency

Usage:
    python build-kb-v2.8.2.py --methodology-only --workers 8
    python build-kb-v2.8.2.py --both --faiss --workers 6
"""
...
class OpenAIKnowledgeBuilder:
    """
    High-performance knowledge base builder with cross-file batching
    """
    ...
    def __init__(self, source_dir: Path, build_mode: str,
        ...
        logger.info(f"🚀 OpenAI Knowledge Builder v2.8.2 initialized:")
    ...
    def build_knowledge_bases(self, rebuild: bool = False):
        ...
        print(f"🚀 Starting CROSS-FILE BATCHING knowledge base build...")
    ...
    def _build_variables_faiss(self, canonical_path: Path):
        ...
        build_info = {
            ...
            'performance_version': 'v2.8.2-crossfile'
        }
    ...
    def _build_methodology_database(self, rebuild: bool = False):
        ...
        # Find documentation files with size filtering
        exclude_patterns = [
        ...
        for f in self.source_dir.rglob('*'):
            if (f.is_file() and
                ...
                if file_size > 5 * 1024 * 1024:  # Skip files > 5MB
                    skipped_files_initial.append({
                        'file': str(f),
                        'size_bytes': file_size,
                        'reason': f'File too large ({file_size} bytes > 5MB)'
                    })
                    continue
                    
                # Skip very large files in test mode
                if self.test_mode and file_size > 1 * 1024 * 1024:
                    skipped_files_initial.append({
                        'file': str(f),
                        'size_bytes': file_size,
                        'reason': f'File too large for test mode ({file_size} bytes > 1MB)'
                    })
                    continue
        ...
        logger.info(f"🚫 Pre-filtered {len(skipped_files_initial)} files (too large)")
            ...
            with open(skipped_log_path, 'w') as f:
                json.dump({
                    ...
                    'summary': {
                        ...
                        'total_files_found': len(doc_files) + len(skipped_files_initial) + len(self.methodology_stats['skipped_files']),
                        'files_processed': self.methodology_stats['files_processed'],
                        'files_skipped': len(self.methodology_stats['skipped_files']) + len(skipped_files_initial)
                    }
                }, f, indent=2)
        ...
        logger.info(f"🚫 Total files skipped: {self.methodology_stats['files_skipped'] + len(skipped_files_initial)}")
    ...
    def _display_final_stats(self, build_time):
        ...
        print(f"\n🎉 CROSS-FILE BATCHING build complete in {build_time:.1f}s!")
...
def main():
    parser = argparse.ArgumentParser(description='High-Performance OpenAI Knowledge Base Builder v2.8.2 - Cross-File Batching')
    ...
    try:
        ...
        logger.info("🚀 Cross-file batching knowledge base build completed successfully!")
